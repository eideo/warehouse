...
<body>
						...
						<table>
							<tr>
								...
								<td style="vertical-align: top;"><select
									class="chzn-select" name="ROLE_ID" id="role_id"
									data-placeholder="请选择商户"
									style="vertical-align: top; width: 120px;">
										...
										<c:forEach items="${clientsList}" var="clients">
											...
											<option value="${clients.DEPT_ID }"
												<c:if test="${clients.DEPT_ID == pd.DEPT_ID }">selected</c:if>>${clients.NAME }</option>
										</c:forEach>
																...
																<c:otherwise>
																	<c:if test="${QX.del == 1 }">
																		<a class='btn btn-mini btn-danger' title="删除"
																			onclick="delUser('${user.StoreHouse_ID }','${user.Address }');"><i
																			class='icon-trash'></i></a>
																	</c:if>
																</c:otherwise>
															</c:choose>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>warehouse/goAddU.do';
			 diag.Width = 225;
			 diag.Height = 415;
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗? ["+userId+"]", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>warehouse/deleteU.do?StoreHouse_ID="+userId+"&tm="+new Date().getTime();
				//	var url2="baidu.com";

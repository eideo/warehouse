...
public class MD5ForBeijing {
    ...
    public MD5ForBeijing(String input, String enc) {
        byte bytes [] = null;
        try {
    ...
    public MD5ForBeijing(String input) {
        this(input, "UTF8");
    }
    ...
    public MD5ForBeijing(InputStream in) {
        this.stringp = false ;
        this.in      = in ;
        this.state   = new int[4] ;
        this.buffer  = new byte[64] ;
    ...
    public void hmac_Md5 ( String text , String key )
        ...
        byte key1 [] ;
        int i ;
        int text_len = text.getBytes("GBK").length ;
        int key_len = key.getBytes().length ;
        key1 = key.getBytes() ;

        if ( key_len > 64 ) {
            key1 = new MD5ForBeijing( key ).getDigest() ;
            key_len = 16 ;
        }
        ...
        MD5ForBeijing md5 = new MD5ForBeijing( "" ) ;
        md5.update( k_ipad , 64 ) ;
        md5.update( text.getBytes("GBK") , text_len ) ;
        digest = md5.getDigest() ;

        MD5ForBeijing md51 = new MD5ForBeijing( "" ) ;
